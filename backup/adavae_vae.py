...
from transformers.models.gpt2.modeling_gpt2 import ACT2FN, GPT2Model, GPT2LMHeadModel
from transformers.models.gpt2.modeling_gpt2 import GPT2Attention as Attention
from transformers.models.gpt2.modeling_gpt2 import GPT2Block as Block
from transformers.models.gpt2.modeling_gpt2 import GPT2MLP as MLP
from transformers.modeling_utils import PreTrainedModel, Conv1D, prune_conv1d_layer, SequenceSummary
sys.path.append('../')
from adavae_stuff import AdapterConfig, init_lisa_params, init_bert_weights, init_bias_mlp, init_zero_weights, \
...
class Cond_Attention(Attention):
    def __init__(self, nx, n_positions, config, AdapterConfig, scale=False):
...
class MAM_Attention(Attention):
    ...
    """
    def __init__(self, nx, n_positions, config, AdapterConfig, add_attn=True, add_mem=False, bias_bool=True, scale=False):
        super(Attention, self).__init__()
        # self.output_attentions = config.output_attentions
        self.output_attentions = False
...
class Unmasked_Block(Block):
    ...
    """
    def __init__(self, n_positions, config, AdapterConfig, scale=False):
        super(Block, self).__init__()
        nx = config.n_embd
        self.ln_1 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.attn = Unmasked_Attention(nx, n_positions, config, scale)
        self.ln_2 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.mlp = MLP(4 * nx, config)

## Additive attention block for method 2 in the paper
class Masked_Block(Block):
    def __init__(self, n_positions, config, AdapterConfig, add_attn=True, add_mem=False, scale=False):
        super(Block, self).__init__()
        nx = config.n_embd
        self.ln_1 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.attn = MAM_Attention(nx, n_positions, config, AdapterConfig, add_attn, add_mem, scale=scale)
        self.ln_2 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.mlp = MLP(4 * nx, config)
...
class Unmasked_AdapterBlock(Block):
    ...
    def __init__(self, n_positions, config, AdapterConfig, scale=False):
        ...
        self.ln_1 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        # self.attn = Unmasked_Attention(nx, n_positions, config, scale)
        self.attn = MAM_Unmasked_Attention(nx, n_positions, config, AdapterConfig, scale=scale)
        self.ln_2 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        if AdapterConfig.ffn_option == "pfeiffer":
            self.ln_3 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        if config.add_cross_attention:
            self.crossattention = Attention(nx, n_positions, config, scale, is_cross_attention=True)
...
class AdapterBlock(Block):
    ...
    def __init__(self, n_positions, config, AdapterConfig, add_attn=True, add_mem=False, scale=False):
        ...
        self.ln_1 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        # self.attn = Cond_Attention(nx, n_positions, config, AdapterConfig, scale)
        self.attn = MAM_Attention(nx, n_positions, config, AdapterConfig, add_attn, add_mem, scale=scale)
        self.ln_2 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        if AdapterConfig.ffn_option == "pfeiffer":
            self.ln_3 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
...
class Encoder(GPT2Model):
    def __init__(self, config, AdapterConfig):
        ...
        self.h = nn.ModuleList([Unmasked_Block(config.n_positions, config, AdapterConfig, scale=True) for _ in range(n)]) if self.tune_enc \
            else nn.ModuleList([Unmasked_AdapterBlock(config.n_positions, config, AdapterConfig, scale=True) for _ in range(n)])
...
class Decoder(GPT2Model):
    def __init__(self, config, AdapterConfig, add_input=False, add_attn=False, add_mem=False, attn_proj_vary=False):
        ...
        if self.add_attn or self.add_mem:
            ...
            self.h = nn.ModuleList([Masked_Block(config.n_positions, config, AdapterConfig, add_attn, add_mem,
                                                 scale=True) for _ in range(n)]) if self.tune_dec else \
                nn.ModuleList([AdapterBlock(config.n_positions, config, AdapterConfig, add_attn, add_mem,
            ...
            n = AdapterConfig.decoder_n_layer
            self.h = nn.ModuleList([Block(config) for _ in range(n)])
            ...
            if self.add_attn or self.add_mem:
                ...
                if self.tune_dec:
                    outputs = block(
                        ...
                        head_mask=head_mask[i], output_attentions=True
                    )
                else:
                    outputs = block(
                        hidden_states, z, layer_past=layer_past, attention_mask=attention_mask,
                        head_mask=head_mask[i], prefix_state=prefix_state[i] if isinstance(prefix_state, list) else prefix_state, output_attentions=True
                    )

            else:
                outputs = block(
                    hidden_states, layer_past=layer_past, attention_mask=attention_mask, head_mask=head_mask[i], output_attentions=True
                )
